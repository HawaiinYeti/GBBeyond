          ...
          ol do
            settings.each do |key, h|
              li do
                ...
                if h[:field_type] == :select
                  f.select key, name: "setting[#{key}]", id: "setting_#{key}" do
                    h[:options].each { |x| f.option x, value: x, selected: (Setting.send(key) == x) }
                  end
                else
...
def settings
  {
    ...
    'play_jwplayer' => { name: 'Play JW Player Videos', field_type: :checkbox, description: 'Allow playing videos with JWPlayer URLs. Starting ~2023-02-01, GB started using the JWPlayer CDN. The JWPlayer URLs provided have short expiry times, requiring the API to be hit every time the video is queued up in order to get a new URL. If you have a lot of channels, this can result in you hitting the hourly API limit. Leaving this setting disabled will prevent new videos from being added to your channel queues.' },
    ...
    'archive_path' => { name: 'Archive Path', field_type: :text, description: 'Local directory to archive videos to. Example: "/Users/user_name/media/gb_videos" or "/mnt/c/gb_videos". You also need to enable the Archive Videos setting on individual channels for videos to save. Docker restart and rebuild required for changes to take effect. Quick restart with `. ./restart.sh` in terminal' },
    'archive_method' => { name: 'Archive Method', field_type: :select, description: 'Method to use for archiving videos. "Playback" will download the videos as they are played on the channels. This will prevent the app from monopolizing your bandwidth in the immediate term, but will take longer to download all the videos marked to be archived. "Continuous" will download unarchived videos whenever there isn\'t already a download occurring, whether it is currently playing on the channel or not. This will download the videos faster overall, but may monopolize your bandwidth and use more host machine resources.', options: ['Playback', 'Continuous'] },
    'hours_to_enqueue' => { name: 'Hours to Enqueue', field_type: :number, description: 'Number of hours of videos to queue up for each channel.' },
  }
end